...
const gambleOptions = [{name: '48x - 0.3% winrate', value: '48x'}, {
    name: '24x - 0.6% winrate',
    value: '24x'
}, {name: '12x - 1.2% winrate', value: '12x'}, {name: '6x - 2.4% winrate', value: '6x'}, {
    name: '3x - 4.8% winrate',
    value: '3x'
}, {name: '1.5x - 9.6% winrate', value: '1.5x'}];
...
client.on('messageCreate', async message => {
    ...
    const forbiddenWords = ['league of legends', 'league', 'favonius', 'fav', 'valorant', 'riot', 'val'];
    const likedWords = ['kafka', 'thigh', 'liyue', 'nefer', 'yelan', 'eula', 'columbina', 'jahoda'];
    ...
    try {
        ...
        console.error("There was an error processing words in message: " + error);
    }
});
...
client.on('interactionCreate', async interaction => {
    ...
    try {
        ...
        if (interaction.isCommand()) {
            ...
            if (commandName === 'cai') {
                ...
                try {
                    ...
                    await interaction.editReply({
                        content: 'There was an error while communicating with Character.ai.',
                        ephemeral: true
                    });
                }
            }
            ...
            if (commandName === 'reset') {
                ...
                try {
                    ...
                    await interaction.editReply({
                        content: 'There was an error while trying to reset the chat history.',
                        ephemeral: true
                    });
                }
            }
            ...
            if (commandName === 'liyue-credit') {
                ...
                if (subcommand === 'remove') {
                    ...
                    if (interaction.user.id !== '470603317228208142') {
                        if (amount > settings.amountLimit && settings.amountLimit !== -1) { //-1 indicates no limit
                            return interaction.editReply({
                                content: `You can't take more than ${settings.amountLimit} liyue credits at a time!`,
                                ephemeral: true
                            });
                        }
                        ...
                        if (amount < 0 && !settings.negativeCredits) {
                            return interaction.editReply({
                                content: "You can't remove negative numbers, use add command instead!",
                                ephemeral: true
                            });
                        }
                    }
                    ...
                    if (interaction.user.id !== '470603317228208142') {
                        if (amount > settings.amountLimit && settings.amountLimit !== -1) {
                            return interaction.editReply({
                                content: `You can't add more than ${settings.amountLimit} liyue credits at a time!`,
                                ephemeral: true
                            });
                        }
                        ...
                        if (amount < 0 && !settings.negativeCredits) {
                            return interaction.editReply({
                                content: "You can't add negative numbers, use remove command instead!",
                                ephemeral: true
                            });
                        }
                    }
                    ...
                    if (balance < 1) {
                        return interaction.editReply({
                            content: "In terms of mora, you have no mora lol",
                            ephemeral: true
                        });
                    }
                    if (wager > balance) {
                        return interaction.editReply({
                            content: "Your wager cannot be higher than your credits balance",
                            ephemeral: true
                        });
                    }
                    if (wager <= 0) {
                    ...
                    if (doubleOdds) {
                        if (getChance(odds * 2)) {
                            ...
                            return interaction.editReply({
                                content: `You won the double-odds roll!! and got ${potentialwinnings} liyue credits.`,
                                ephemeral: true
                            });
                        }
                        liyueCredits.removeCredits(user.id, (potentialwinnings) - wager, interaction.guild.id, false);
                        return interaction.editReply({
                            content: `You lost the double-odds roll and lost ${potentialwinnings} liyue credits.`,
                            ephemeral: true
                        });
                        ...
                        if (getChance(odds)) {
                            liyueCredits.addCredits(user.id, potentialwinnings, interaction.guild.id, false);
                            return interaction.editReply({
                                content: `You won the roll!! and got ${potentialwinnings} liyue credits.`,
                                ephemeral: true
                            });
                        }
                        return interaction.editReply({
                            content: `You lost the roll and lost ${wager} liyue credits.`,
                            ephemeral: true
                        });
                    }
                    ...
                    return interaction.editReply({
                        content: `${user.username} has ${credits.toLocaleString('en-US')} Liyue credits.`,
                        ephemeral: true
                    });
                    ...
                    if (balance < 200000) {
                        return interaction.editReply({
                            content: "You need 200,000 credits to cause an earthquake!",
                            ephemeral: true
                        });
                    }
                    //take the fee
                    ...
                    return interaction.editReply({
                        content: "Earthquake! some credits have been shaken up!",
                        ephemeral: true
                    });
                } else if (subcommand === 'settings') {
                    if (interaction.user.id !== '144828640146882560') {
                        return interaction.editReply({
                            content: "You don't have permission to edit settings",
                            ephemeral: true
                        });
                    }
            ...
            if (action === 'accept' || action === 'join') {
                ...
                await interaction.update({
                    content: `Game started between ${challenger.username} and ${opponent.username}!`,
                    components: []
                });
                game.startGame();
            }
        ...
        console.error("There was an error processing command: " + commandName);
        console.error(error);
    }
});
